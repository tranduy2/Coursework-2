...
const ConfirmationScreen = ({ navigation, route }) => {
  // Lấy dữ liệu từ màn hình AddHike
  ...
  const handleConfirm = async () => {
    try {
      ...
      // Quay về màn hình Home (xóa AddHike và Confirmation khỏi stack)
      navigation.popToTop();
    } catch (error) {
      console.error(error);
  ...
  const handleEdit = () => {
    // Quay lại màn hình trước đó (AddHikeScreen)
    navigation.goBack();
  };
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Text style={styles.value}>
        {hikeData.description || 'N/A'}
      </Text>
      
      <Text style={styles.label}>Custom Field 1:</Text>
      <Text style={styles.value}>
        {hikeData.custom_field1 || 'N/A'}
      </Text>
      
      <Text style={styles.label}>Custom Field 2:</Text>
      <Text style={styles.value}>
        {hikeData.custom_field2 || 'N/A'}
      </Text>
